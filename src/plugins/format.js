...
exports.FormatPlugin = void 0;
const plugin_models_1 = require("../plugin_models");
// Define STYLE_CODES constant as specified
const STYLE_CODES = {
    [plugin_models_1.Style.BOLD]: "**",
    [plugin_models_1.Style.ITALICS]: "__",
    [plugin_models_1.Style.CODE]: "`",
    [plugin_models_1.Style.STRIKE]: "~~",
    [plugin_models_1.Style.PLAIN]: "", // For PLAIN, no wrapping characters are applied
    [plugin_models_1.Style.PRESERVE]: "" // PRESERVE also doesn't wrap, it's handled by early exit
};
class FormatPlugin {
    constructor(config) {
        this.id = "fmt";
        ...
        if (!this.config.style) {
            this.config.style = plugin_models_1.Style.PRESERVE;
        }
        console.log('FormatPlugin initialized with config:', JSON.stringify(this.config, null, 2));
    }
    modify(tm) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!tm.text || !this.config.style || this.config.style === plugin_models_1.Style.PRESERVE) {
            ...
            if (this.config.style === plugin_models_1.Style.PLAIN) {
