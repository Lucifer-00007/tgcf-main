...
exports.messageMappings = void 0;
exports.storeMessageMapping = storeMessageMapping;
exports.getForwardedMessage = getForwardedMessage;
exports.getForwardedMessages = getForwardedMessages;
exports.removeMessageMapping = removeMessageMapping;
exports.loadMappingsFromDB = loadMappingsFromDB;
exports.saveMappingsToDB = saveMappingsToDB;
...
exports.messageMappings = new Map();
 ...
 */
function storeMessageMapping(originalChatId, originalMessageId, destChatId, forwardedMessageId) {
    const key = `${originalChatId}-${originalMessageId}`;
    if (!exports.messageMappings.has(key)) {
        exports.messageMappings.set(key, new Map());
    }
    const destinationMap = exports.messageMappings.get(key);
    ...
    if (exports.messageMappings.size > KEEP_LAST_MANY) {
        // Delete the oldest entry (maps are iterated in insertion order)
        const oldestKey = exports.messageMappings.keys().next().value;
        if (oldestKey) {
            exports.messageMappings.delete(oldestKey);
 ...
 */
function getForwardedMessage(originalChatId, originalMessageId, destChatId) {
    const key = `${originalChatId}-${originalMessageId}`;
    const destinationMap = exports.messageMappings.get(key);
    if (destinationMap) {
 ...
 */
function getForwardedMessages(originalChatId, originalMessageId) {
    const key = `${originalChatId}-${originalMessageId}`;
    return exports.messageMappings.get(key);
}
/**
