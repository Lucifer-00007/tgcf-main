...
exports.stopLiveMode = stopLiveMode;
const grammy_1 = require("grammy");
const config_1 = require("./config");
const telegram_utils_1 = require("./telegram_utils");
const loader_1 = require("./plugins/loader");
const processState_1 = require("./processState"); // Import to update state on external stop
let liveBotInstance = null;
function startSync() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const liveConfig = (0, config_1.getConfig)();
        if (!liveConfig.login.BOT_TOKEN) {
            console.error('LIVE: Bot token is not configured. Live mode cannot start.');
            (0, processState_1.setProcessState)('idle', 'Bot token not configured for live mode.', 'BOT_TOKEN_MISSING');
            throw new Error('Bot token is not configured for live mode.');
        }
        console.log('LIVE: Initializing Bot...');
        liveBotInstance = new grammy_1.Bot(liveConfig.login.BOT_TOKEN);
        ...
        yield (0, loader_1.loadPlugins)(liveBotInstance, liveConfig.plugins);
        console.log('LIVE: Loading forwarding map (from-to)...');
        const fromToMap = yield (0, config_1.loadFromTo)(liveBotInstance, liveConfig.forwards);
        if (fromToMap.size === 0) {
        ...
        const adminIds = yield (0, config_1.loadAdmins)(liveBotInstance, liveConfig.admins);
        ...
        try {
            const botInfo = yield liveBotInstance.api.getMe();
            console.log(`LIVE: Bot info received: ${botInfo.username} (ID: ${botInfo.id})`);
            const commands = [
                { command: 'start', description: ((_a = liveConfig.bot_messages) === null || _a === void 0 ? void 0 : _a.start) || 'Start the bot' },
                { command: 'ping', description: 'Check if the bot is alive' },
                { command: 'help', description: ((_b = liveConfig.bot_messages) === null || _b === void 0 ? void 0 : _b.bot_help) || 'Show help message' },
            ];
            yield liveBotInstance.api.setMyCommands(commands);
        ...
        liveBotInstance.on('message', (ctx) => __awaiter(this, void 0, void 0, function* () {
            var _a, _b;
            ...
            if (fromToMap.has(chatId)) {
                ...
                let tgcfMessage;
                try {
                    tgcfMessage = yield (0, loader_1.applyPlugins)(liveBotInstance, message);
                    if (!tgcfMessage) {
                        console.log(`LIVE: Message ${message.message_id} from ${chatId} was filtered out by plugins.`);
                        return;
                    }
                    for (const destChatId of destinations) {
                        yield (0, telegram_utils_1.sendMessage)(liveBotInstance, destChatId, tgcfMessage.originalMessage, chatId, liveConfig.show_forwarded_from, tgcfMessage);
                    }
                }
                catch (error) {
                    ...
                    yield (0, telegram_utils_1.cleanupTgcfMessageFile)(tgcfMessage);
                }
            }
            if (message.text) {
                if (message.text.startsWith('/ping'))
                    ...
                    yield ctx.reply(((_a = liveConfig.bot_messages) === null || _a === void 0 ? void 0 : _a.start) || 'Hello! I am operational.');
                else if (message.text.startsWith('/help'))
                    yield ctx.reply(((_b = liveConfig.bot_messages) === null || _b === void 0 ? void 0 : _b.bot_help) || 'No help configured yet.');
            }
        }));
        liveBotInstance.on('edited_message', (ctx) => __awaiter(this, void 0, void 0, function* () {
            const chatId = ctx.chat.id;
            ...
            if (fromToMap.has(chatId)) {
                ...
                let tgcfMessage;
                try {
                    tgcfMessage = yield (0, loader_1.applyPlugins)(liveBotInstance, editedMessage);
                    if (!tgcfMessage) {
