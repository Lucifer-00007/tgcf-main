...
exports.stopPastMode = stopPastMode;
const grammy_1 = require("grammy");
const config_1 = require("./config"); // Renamed to avoid conflict
const telegram_utils_1 = require("./telegram_utils");
const loader_1 = require("./plugins/loader");
const processState_1 = require("./processState"); // Import to check state
...
function forwardJob() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!config.login.BOT_TOKEN) {
            ...
            (0, processState_1.setProcessState)('idle', 'Bot token not configured for past mode.', 'BOT_TOKEN_MISSING');
        ...
        const fromToMap = yield loadFromTo(bot, config.forwards);
        if (fromToMap.size === 0) {
            console.warn('PAST: No valid forward rules were resolved. Past mode will not process any messages.');
            (0, processState_1.setProcessState)('idle', 'No forward rules for past mode.');
            return;
        }
        ...
        for (const [sourceChatId, { destinations, forwardConfig }] of fromToMap) {
            ...
            while (continueFetching) {
                ...
                try {
                    const history = yield bot.api.getChatHistory(sourceChatId, {
                    ...
                    for (const rawMessage of messagesToProcess) {
                        ...
                        let tgcfMessage;
                        try {
                            tgcfMessage = yield (0, loader_1.applyPlugins)(bot, rawMessage);
                            if (!tgcfMessage)
                                continue;
                            for (const destChatId of destinations) {
                                yield (0, telegram_utils_1.sendMessage)(bot, destChatId, tgcfMessage.originalMessage, sourceChatId, config.show_forwarded_from, tgcfMessage);
                            }
                            if (config.past.delay && config.past.delay > 0) {
                                yield delay(config.past.delay);
                            }
                        }
                        catch (pluginError) {
                    ...
                    if (error instanceof grammy_1.GrammyError) {
                        if (error.error_code === 429 || ((_a = error.parameters) === null || _a === void 0 ? void 0 : _a.retry_after)) {
                            const retryAfter = ((_b = error.parameters) === null || _b === void 0 ? void 0 : _b.retry_after) || 60;
