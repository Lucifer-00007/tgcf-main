...
function LoginPage() {
    const [step, setStep] = (0, react_1.useState)('enterPhoneNumber');
    ...
    const [isInitialStatusLoading, setIsInitialStatusLoading] = (0, react_1.useState)(true);
    ...
    (0, react_1.useEffect)(() => {
        ...
        const checkCurrentStatus = () => __awaiter(this, void 0, void 0, function* () {
    ...
    const handlePhoneNumberSubmit = (e) => __awaiter(this, void 0, void 0, function* () {
        e.preventDefault();
        setIsLoading(true);
        setError(null);
        setSuccessMessage(null);
        try {
            const response = yield (0, apiClient_1.initiateLogin)(phoneNumber);
    ...
    const handleCodeSubmit = (e) => __awaiter(this, void 0, void 0, function* () {
        e.preventDefault();
        if (!sessionId) {
        ...
        setSuccessMessage(null);
        try {
            const response = yield (0, apiClient_1.submitCode)(sessionId, code);
    ...
    const handlePasswordSubmit = (e) => __awaiter(this, void 0, void 0, function* () {
        e.preventDefault();
        if (!sessionId) {
            setError('Session ID is missing.');
            setStep('error');
        ...
        setSuccessMessage(null);
        try {
            const response = yield (0, apiClient_1.submitPassword)(sessionId, password);
            setSuccessMessage(response.message || 'Login successful!');
            setStep('loggedIn');
        }
        catch (err) {
    ...
    return (<div className="container mx-auto p-4 max-w-md">
      <div className="bg-gray-800 shadow-xl rounded-lg p-6 md:p-8">
        ...
        {step === 'loggedIn' && (<div className="text-center">
            ...
            <button onClick={resetFlow} className="mt-6 w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition duration-150">
                Log Out / Start Over
            </button>
          </div>)}

        {step === 'enterPhoneNumber' && (<form onSubmit={handlePhoneNumberSubmit} className="space-y-6">
            <div>
              <label htmlFor="phoneNumber" className="block text-sm font-medium text-gray-300">
                Phone Number
              </label>
              <input type="tel" id="phoneNumber" name="phoneNumber" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} required placeholder="+12345678900" className="mt-1 block w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"/>
            </div>
            <button type="submit" disabled={isLoading} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition duration-150 disabled:opacity-50">
              {isLoading ? 'Sending...' : 'Send Code'}
            </button>
          </form>)}

        {step === 'enterCode' && (<form onSubmit={handleCodeSubmit} className="space-y-6">
            <div>
              <label htmlFor="code" className="block text-sm font-medium text-gray-300">
                Verification Code
              </label>
              <input type="text" id="code" name="code" value={code} onChange={(e) => setCode(e.target.value)} required placeholder="12345" className="mt-1 block w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"/>
            </div>
            <button type="submit" disabled={isLoading} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition duration-150 disabled:opacity-50">
        ...
        {step === 'enterPassword' && (<form onSubmit={handlePasswordSubmit} className="space-y-6">
            <div>
              <label htmlFor="password" className="block text-sm font-medium text-gray-300">
                Two-Factor Authentication Password
              </label>
              <input type="password" id="password" name="password" value={password} onChange={(e) => setPassword(e.target.value)} required placeholder="Your 2FA password" className="mt-1 block w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"/>
            </div>
            <button type="submit" disabled={isLoading} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition duration-150 disabled:opacity-50">
