...
import type { Config, Forward } from '../../../../src/config'; // Import types

// Define a local type for the form, as Forward might have optional fields
// that are required in the form until submitted.
interface ConnectionFormState {
  con_name: string;
  use_this: boolean;
  source: string; // Input as string
  dest: string;   // Comma-separated string
  offset: string; // Input as string, optional
  end: string;    // Input as string, optional
}
...
export default function ConnectionsPage() {
  ...
  const [connections, setConnections] = useState<Forward[]>([]);
  const [newConnectionForm, setNewConnectionForm] = useState<ConnectionFormState>(initialFormState);
  
  const [isLoading, setIsLoading] = useState(true);
  const [isSaving, setIsSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [successMessage, setSuccessMessage] = useState<string | null>(null);
  const [editingIndex, setEditingIndex] = useState<number | null>(null); // To edit existing connection
  ...
  const handleFormChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (type === 'checkbox') {
        ...
        setNewConnectionForm(prev => ({ ...prev, [name]: checked }));
    } else {
        setNewConnectionForm(prev => ({ ...prev, [name]: value }));
    }
  };
  
  const parseInputToNumberOrString = (input: string): number | string => {
    return /^\d+$/.test(input.trim()) ? parseInt(input.trim(), 10) : input.trim();
  };

  const handleAddOrUpdateConnection = (e: FormEvent) => {
    e.preventDefault();
    setError(null);
    ...
    if (editingIndex !== null) {
      updatedConnections = connections.map((conn, index) => 
        index === editingIndex ? newForwardRule : conn
      );
  ...
  const handleEditConnection = (index: number) => {
    ...
    setNewConnectionForm({
      ...
      offset: connToEdit.offset?.toString() || '',
      end: connToEdit.end === null ? '' : (connToEdit.end?.toString() || ''),
    });
  ...
  const handleSaveChanges = async () => {
    ...
    setSuccessMessage(null);
    try {
      const updatedConfig = { ...fullConfig, forwards: connections };
      ...
      <div className="bg-gray-800 shadow-xl rounded-lg p-6 md:p-8">
        ...
        <form onSubmit={handleAddOrUpdateConnection} className="mb-10 p-6 bg-gray-750 rounded-lg shadow space-y-4">
          ...
          <div className="flex justify-end space-x-3">
            {editingIndex !== null && (
                <button type="button" onClick={() => { setEditingIndex(null); setNewConnectionForm(initialFormState); setError(null); setSuccessMessage(null); }} className="secondary-button">Cancel Edit</button>
            )}
        ...
        <div className="mb-8">
          ...
          {connections.length === 0 ? (
            <p className="text-gray-400 text-center">No connections configured yet. Add one using the form above.</p>
          ) : (
            <div className="space-y-4">
              {connections.map((conn, index) => (
                <div key={index} className={`p-4 rounded-lg shadow ${conn.use_this ? 'bg-gray-700' : 'bg-gray-600 opacity-70'}`}>
