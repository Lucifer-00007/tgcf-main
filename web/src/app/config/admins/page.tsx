...
import type { Config } from '../../../../src/config'; // Import the Config type

export default function AdminsPage() {
  const [fullConfig, setFullConfig] = useState<Config | null>(null);
  const [admins, setAdmins] = useState<(number | string)[]>([]);
  const [newAdmin, setNewAdmin] = useState<string>(''); // Input is always string
  ...
  const handleAddAdmin = (e: FormEvent) => {
    ...
    const adminToAdd: number | string = /^\d+$/.test(newAdmin.trim()) 
  ...
  const handleRemoveAdmin = (adminToRemove: number | string) => {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      const updatedConfig = { ...fullConfig, admins: admins };
      ...
      <div className="bg-gray-800 shadow-xl rounded-lg p-6 md:p-8">
        ...
        <form onSubmit={handleAddAdmin} className="mb-8 flex flex-col sm:flex-row items-stretch gap-3">
          <input
            ...
            className="flex-grow mt-1 block w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
          />
          <button
            type="submit"
            className="bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-4 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50 transition duration-150 whitespace-nowrap"
          >
            Add Admin
          </button>
        </form>
          ...
          {admins.length === 0 ? (
            ...
            <ul className="space-y-2">
              {admins.map((admin, index) => (
                <li
                  key={index}
                  className="flex items-center justify-between bg-gray-700 p-3 rounded-md shadow"
                >
                  <span className="text-gray-200">{admin}</span>
                  <button
                    onClick={() => handleRemoveAdmin(admin)}
                    className="bg-red-600 hover:bg-red-700 text-white font-semibold py-1 px-3 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-50 transition duration-150"
                  >
                    Remove
                  </button>
                </li>
        ...
        <div className="mt-10 text-center">
          <button
            ...
            disabled={isSaving || isLoading} // Also disable if initial load is happening
